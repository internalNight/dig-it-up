...
{
    const float PathRun=FrontPosition-Rear, PathLoop=2*PathRun+2*PI*Radius;
    ...
    else if (S < PathRun + PI*Radius)
    {
        const float A = (S-PathRun)/Radius;
        Position = FVector3f(Rear-Radius*FMath::Sin(A),0,Top-Radius+Radius*FMath::Cos(A));
        Tangent = FVector3f(-FMath::Cos(A),0,-FMath::Sin(A));
    }
    else if (S < 2*PathRun + PI*Radius)
    {
        Position = FVector3f(Rear+S-PathRun-PI*Radius,0,Top-2*Radius);
        Tangent = FVector3f(1,0,0);
    }
    else
    {
        const float A = (S-2*PathRun-PI*Radius)/Radius;
        Position = FVector3f(FrontPosition+Radius*FMath::Sin(A),0,Top-Radius-Radius*FMath::Cos(A));
        Tangent = FVector3f(FMath::Cos(A),0,FMath::Sin(A));
    }
}
}
